...
void setup() {
  ...
  levels.get(0).addCoin(new PVector(250, height-570), 20); //Add a coin to the level with the addCoin function
  levels.get(0).addCoin(new PVector(430, height-250), 20); //Arguments for addCoin: 1) PVector defining position of coin, 2) size of coin
  levels.get(0).addObjective(620, height-100, 1); 

  //LEVEL TWO
  levels.add(new Level(200, height-370-playerHeight/2));
  levels.get(1).addPlatform(new PVector(50, height-200), 400);
  levels.get(1).addPlatform(new PVector(150, height-350), 160);
  levels.get(1).addPlatform(new PVector(360, height-480), 230);
  levels.get(1).addWall(new PVector(400, height-200), 280);
  levels.get(1).addCoin(new PVector(170, height-230), 20);
  levels.get(1).addCoin(new PVector(270, height-230), 20);
  levels.get(1).addObjective(500, height-550, 2);
  ...
  levels.get(2).addPlatform(new PVector(100, height-400), 400);
  ...
  levels.get(2).addCoin(new PVector(350, height-470), 20);
  levels.get(2).addObjective(260, height-500, -1);
}
...
void draw() {
  ...
  case 0: //LEVEL ONE
    background(255);
    image(img1, 350, 390, width, height);
  ...
  case 1: //LEVEL TWO
    background(255);
    image(img2, 350, 390, width, height);
    ...
    break;
  case 2: //LEVEL THREE
    image(img3, 350, 350, width, height);
    if (checkObjective(mode)) {
      break;
    }
    levels.get(mode).display();
...
class Player {
  ...
  void display() {
    int whichSprite  = floor(pos.x/10)%sprite.length/2;
    imageMode(CENTER);
    if (keyCode == LEFT) {
...
boolean checkObjective(int i) {
  ...
  if (!changeLevel) {
    ...
    for (int j=0; j<millis()-timeStamp; j=j+80) {
      fill(255, random(170), random(255));
      noStroke();
      ellipse(player.pos.x + random(-playerWidth/2, playerWidth/2), player.pos.y + random(-playerHeight/2, playerHeight/2), random(30), random(30));
    }
  } else {
